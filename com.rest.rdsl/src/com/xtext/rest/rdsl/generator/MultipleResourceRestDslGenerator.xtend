...
class MultipleResourceRestDslGenerator implements IMultipleResourceGenerator {
	...
	def compileObjects(ObjectParentGenerator obgen, RESTState resource) {
	...
	def compileCollectionObjects(ObjectParentGenerator obgen, RESTState colRes) {
	...
	def extractResourcesAndConfiguration(Iterable<RESTModel> model) {
		val List<RESTState> resources = new ArrayList<RESTState>();
		...
		this.resources = new RESTResourceObjects(resources, dispatcher, gloablTraits, security);
	}
