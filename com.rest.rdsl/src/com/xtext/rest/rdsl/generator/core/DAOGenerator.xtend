...
class DAOGenerator {
	...
	new(IFileSystemAccess fsa,  RESTResourceObjects resourceCol) {
		...
		idDataType = resourceCol.globalTraits.idtype.literal;
	}
	
		
	def generateInterface() {
		for(res: resourceCol.getSingleResources){
	...
	def generateDAOs() {
		...
		for(res: resourceCol.getSingleResources){
			...
			for(attrib : res.resources.get(0).attributes){
			...
			public class «res.name»DAO implements I«res.name»DAO{
				...
				public boolean update(«res.name» «objectName», «idDataType» id) throws Exception{
...
«««						«var String resource = (attrib.value as ResourceReference).resourceRef.name»
...
//		}
		...
		public class «Naming.CLASS_SQLITE»{
			...
			private void connect(){
				try{
«««					«resourceCol.globalTraits..jdbcCon»
««« 				«config.jdbcFile»
		...
		public class «Naming.CLASS_DB_QUERY» extends HashMap<String, Object> {
			...
			public String getWhere(«this.idDataType.toFirstUpper» id){
