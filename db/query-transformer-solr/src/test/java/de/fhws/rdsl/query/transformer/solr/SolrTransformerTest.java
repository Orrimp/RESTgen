...
public class SolrTransformerTest extends AbstractSolrTransformerTest {
	...
	private String getActual(String text, String contextType, List<Object> keys, Long start, Long offset) {
		DefaultQueryTransformer queryTransformer = new DefaultQueryTransformer(DefaultQueryParser::new, SolrNodeTransformer::new, SolrElementTransformer::new,
		...
		try {
			return (String) queryTransformer.transform(new QueryTransformerParameters(query, contextType, schema, Maps.newConcurrentMap())).get("solrQuery");
		} catch (IOException e) {
